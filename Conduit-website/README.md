...
2. **OAuth consent** — partner `redirect_uri`s point at `/oauth/consent`. This SPA serves that route. The JWT is read from `localStorage.conduit_jwt` and (as a fallback for backend-hosted flows) from a `?token=` query parameter.
...
- 401 → clear `conduit_jwt`, cancel queries, redirect to `/auth?redirect=<from>`.
- Partner admin actions attach `X-Partner-Admin-Token` from `localStorage.conduit_partner_token`.
